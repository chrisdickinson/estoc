...
proto._reportFunction = function(fn, parent) {
  this._stream.emit('defn', fn, parent)
}

proto._reportCall = function(from, to) {
  this._stream.emit('callfn', from, to)
}

proto._reportIOCCall = function(from, to) {
  this._stream.emit('ioc-callfn', from, to)
}
