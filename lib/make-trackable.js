...
function makeTrackable(cfg, title, trackProgression) {
  ...
  function makeTrackableFrom(obj, title) {
    var baseCopy = obj.copy
    var baseGetprop = obj.getprop
    var baseDelprop = obj.delprop
    ...
    function delprop(name) {
      var target = makeTrackableFrom(cfg.makeFunction(callSpy), title.concat([name]))
      var marks = this.getMark('tracked-name')
      trackProgression(new Usage(Usage.DELETE, cfg, marks[0]))
      return baseDelprop.call(this, arguments)
    }

    function getprop(name) {
      var baseName = baseGetprop.call(this, arguments)
      if (!baseName) {
