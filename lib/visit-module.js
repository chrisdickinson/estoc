...
function visitModule(filename, ast, visitor, exports, ready) {
  ...
  moduleWrapper._name = '<module ' + JSON.stringify(filename) + '>'
  ...
  function _advanceCFG() {
    ...
    while (1) {
      ...
      if (++count === visitor.maxIterations) {
        return process.nextTick(advanceCFG)
      }
    }
    completeCFG()
  }
  ...
  function visitUnexecuted(cb) {
    ready = once(cb)
    visitor.activate()
    process.nextTick(iterateUnexecuted)
  }
  ...
  function iterateUnexecuted() {
    ...
    if (marks.length) for (var i = 0; i < arity; ++i) {
      args[i] = makeTrackable(
        cfg,
        marks[0].getMark('tracked-name')[0].concat(Usage.makeIOCSegment(i)),
        visitor.reportUsage
      )
    ...
    for (var xs of nextFn._references) {
      var source = xs.getCurrentSourceObject()
      if (source === nextFn.getprop('prototype').value()) {
        continue
      }
    ...
    var context = sourceFunction && sourceFunction.makeNew ? sourceFunction.makeNew() : cfg.makeUnknown()

    if (marks.length) {
      visitor._reportIOCCall(marks[0], nextFn)
    }
    nextFn.call(cfg, context, args)
    advanceCFG()
  }
  ...
  function onfunction_collectFunctions(fn, astNode) {
    ...
    visitor._reportFunction(fn, frame.getFunction())
    if (frame) {
  ...
  function oncall(fn, context, args, recursion) {
    var sfi = null
    visitor._reportCall(cfg._callStack.current().getFunction(), fn)
    if (fn.sharedFunctionInfo && (sfi = fn.sharedFunctionInfo())) {
      return sfi.callCount() < visitor.maxCalls
    }
  }

  function oncalled(fn, context, args, recursion) {
  }
